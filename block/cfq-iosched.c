...
void cfq_mark_cfqq_split_coop(struct cfq_queue *cfqq)
{
        (cfqq)->flags |= (1 << CFQ_CFQQ_FLAG_split_coop);
}
static inline void cfq_clear_cfqq_split_coop(struct cfq_queue *cfqq)
{
        (cfqq)->flags &= ~(1 << CFQ_CFQQ_FLAG_split_coop);
}
static inline int cfq_cfqq_split_coop(const struct cfq_queue *cfqq)
{
        if (cfqq->seeky_start &&
		       ...
		       struct cfq_io_context *cic)
{
	...
	if (cfqq->queued[0] + cfqq->queued[1] >= 4)
		...
		enable_idle = 0;
	else if (!atomic_read(&cic->ioc->nr_tasks) ||
		!cfqd->cfq_slice_idle ||

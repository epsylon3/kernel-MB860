...
static struct workqueue_struct *system_nrt_wq = NULL;
...
static irqreturn_t tegra_dc_hdmi_irq(int irq, void *ptr)
{
	...
	if (hdmi->suspended) {
		...
		if (!system_nrt_wq)
			pr_warning("hdmi: delayed work ignored, bad workqueue");
		else if (tegra_dc_hdmi_hpd(dc))
...
static void tegra_dc_hdmi_enable(struct tegra_dc *dc)
{
	...
	// this system workqueue doesnt exists in 2.6.32, create a special one
	system_nrt_wq = create_workqueue("dc-hdmi");
	if (!system_nrt_wq) {
		pr_err("hdmi: Couldn't create workqueue");
	}

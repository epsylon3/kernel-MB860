...
{
	mot_tcmd_platform_data.gpio_list[TCMD_GPIO_ISL29030_INT] = TEGRA_GPIO_PE1;
	mot_tcmd_platform_data.gpio_list[TCMD_GPIO_KXTF9_INT]    = TEGRA_GPIO_PV3;
	mot_tcmd_platform_data.gpio_list[TCMD_GPIO_MMC_DETECT]   = TEGRA_GPIO_PI5;
	mot_tcmd_platform_data.gpio_list[TCMD_GPIO_INT_MAX_NUM]  = -1;
	mot_tcmd_platform_data.size = TCMD_GPIO_INT_MAX_NUM;

	if (machine_is_etna() && system_rev == 0x1100) {
		mot_tcmd_platform_data.gpio_list[TCMD_GPIO_KXTF9_INT] = TEGRA_GPIO_PN4;
	}
...
//	int i = 0;
	 ...
	 * FIXME: Add eMMC support (and make it report valid informations :p)
	 */
/*
	for (i = 0; i < 12; i++) {
